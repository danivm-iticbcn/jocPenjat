...
document.getElementById('formularioHeader').addEventListener('submit', function (event) {
    ...
    obtenirParaulaSecreta(entradaParaula.value.toUpperCase());
});


////////FUNCIONS\\\\\\\\

function obtenirParaulaSecreta(tema) {
    fetch(`http://127.0.0.1:8000/penjat/tematica/${tema}`)
        .then(resposta => resposta.json())
        .then(function (resposta) {
            paraulaSecreta = resposta[0]["paraula"].toUpperCase();
